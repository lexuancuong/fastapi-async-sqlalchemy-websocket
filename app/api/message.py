...
class ConnectionManager:
    ...
    async def broadcast(self, message: Dict):
        ...
        for connection in self.active_connections_dict:
            ...
            if (
                tmp_websocket.application_state == WebSocketState.CONNECTED
...
async def get(request: Request):
    return templates.TemplateResponse(
        "index.html", {"request": request, "domain": settings.DOMAIN}
    )
...
async def websocket_endpoint(
    ...
    user = await get_current_user(async_session=async_db_connection, token=token)
    user_id = user.id
            ...
            try:
                ...
                if data.get("request_type") == RequestType.MESSAGE:
                    ...
                    message_db = await crud.create_message(
                        db_session=async_db_connection, user_id=user_id, text=text
                    )
                    ...
                    data = {"response_type": ResponseType.MESSAGES, "data": list_data}
                    await manager.send_personal_message(data=data, websocket=websocket)
            except:
                pass
